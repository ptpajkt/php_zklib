...
function zkgetattendance($self) {
    ...
    if (getSizeAttendance($self)) {
        ...
        $maxLoops = 20; // batas loop supaya tidak hang
        while ($bytes > 0 && $maxLoops-- > 0) {
    ...
    if (count($self->attendancedata) > 0) {
        for ($x = 0; $x < count($self->attendancedata); $x++) {
            if ($x > 0) $self->attendancedata[$x] = substr($self->attendancedata[$x], 8);
        }
        $attendancedata = substr(implode('', $self->attendancedata), 10);
